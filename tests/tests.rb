# typed: true
# frozen_string_literal: true

require_relative './base_client_test'
require_relative './test_card_token'
require_relative './test_card'
require_relative './test_identification_type'
require_relative './test_merchant_order'
require_relative './test_payment_methods'
require_relative './test_payment'
require_relative './test_preference'
require_relative './test_user'
require_relative './test_preapproval'
require_relative './test_preapproval_plan'
require_relative './test_order'
require_relative './test_order_transaction'
require_relative './test_customer'
require_relative './test_refund'
require_relative './test_subscription'
require_relative './test_advanced_payment'
require_relative './test_chargeback'
require_relative './test_invoice'
require_relative './test_point'
require_relative './test_oauth'
require_relative './test_disbursement_refund'

# TODO Uncomment the following lines when adjusting the tests and refund service
# require_relative './test_refund_with_custom_headers'
